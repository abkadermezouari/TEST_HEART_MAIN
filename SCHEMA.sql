...
SELECT title, is_published, is_featured FROM public.recipes;
...
SELECT id, title, is_published, is_featured, image_url
FROM public.recipes
ORDER BY title;
...
SET name = 'Flocons d''Avoine Entiers Bio'
WHERE name = 'Flocons d''avoine';

UPDATE public.products
SET name = 'Lentilles Vertes Sèches'
WHERE name = 'Lentilles vertes bio';

UPDATE public.products
SET name = 'Pois Chiches Secs Bio'
WHERE name = 'Pois chiches en conserve';




-- ── PRODUITS — URLs permanentes ───────────────────────────────

UPDATE public.products SET image_url =
  'https://images.unsplash.com/photo-1612257999756-c9e4da1b1e99?w=400'
WHERE name ILIKE '%lentill%';

UPDATE public.products SET image_url =
  'https://images.unsplash.com/photo-1515543237350-b3eea1ec8082?w=400'
WHERE name ILIKE '%pois chich%';

UPDATE public.products SET image_url =
  'https://images.unsplash.com/photo-1536304993881-ff86e0c9c7ce?w=400'
WHERE name ILIKE '%riz%';

UPDATE public.products SET image_url =
  'https://images.unsplash.com/photo-1509440159596-0249088772ff?w=400'
WHERE name ILIKE '%pain%';

UPDATE public.products SET image_url =
  'https://images.unsplash.com/photo-1614961233913-a5113a4a34ed?w=400'
WHERE name ILIKE '%flocon%' OR name ILIKE '%avoine%';

UPDATE public.products SET image_url =
  'https://images.unsplash.com/photo-1571212515416-fca988083d3c?w=400'
WHERE name ILIKE '%yaourt%';

UPDATE public.products SET image_url =
  'https://images.unsplash.com/photo-1486297678162-eb2a19b0a318?w=400'
WHERE name ILIKE '%fromage blanc%';

UPDATE public.products SET image_url =
  'https://images.unsplash.com/photo-1598170845058-32b9d6a5da37?w=400'
WHERE name ILIKE '%carott%';

UPDATE public.products SET image_url =
  'https://images.unsplash.com/photo-1523049673857-eb18f1d7b578?w=400'
WHERE name ILIKE '%avocat%';

UPDATE public.products SET image_url =
  'https://images.unsplash.com/photo-1568702846914-96b305d2aaeb?w=400'
WHERE name ILIKE '%pomm%';

UPDATE public.products SET image_url =
  'https://images.unsplash.com/photo-1548839140-29a749e1cf4d?w=400'
WHERE name ILIKE '%eau min%';

UPDATE public.products SET image_url =
  'https://images.unsplash.com/photo-1587049352846-4a222e784d38?w=400'
WHERE name ILIKE '%miel%';

-- ── RECETTES — URLs permanentes ───────────────────────────────

UPDATE public.recipes SET image_url =
  'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=400'
WHERE title ILIKE '%lentill%';

UPDATE public.recipes SET image_url =
  'https://images.unsplash.com/photo-1517673132405-a56a62b18caf?w=400'
WHERE title ILIKE '%porridge%';

UPDATE public.recipes SET image_url =
  'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=400'
WHERE title ILIKE '%riz%' OR title ILIKE '%poulet%';

UPDATE public.recipes SET image_url =
  'https://images.unsplash.com/photo-1638176067095-17c88e0238f9?w=400'
WHERE title ILIKE '%smoothie%' OR title ILIKE '%avocat%';

-- ── VÉRIFICATION ──────────────────────────────────────────────
SELECT name, LEFT(image_url, 60) AS url_debut
FROM public.products ORDER BY name;

SELECT title, LEFT(image_url, 60) AS url_debut
FROM public.recipes ORDER BY title;
...
-- Produits : image unique par produit via son UUID
UPDATE public.products
SET image_url = 'https://picsum.photos/seed/' || id::text || '/600/400'
...
SELECT id, name, image_url FROM public.products WHERE image_url LIKE '%unsplash.com%';
SELECT id, title, image_url FROM public.recipes WHERE image_url LIKE '%unsplash.com%';


-- Voir les URLs actuelles
...
UPDATE public.products
SET image_url = 'https://picsum.photos/seed/' || id::text || '/600/400'
WHERE image_url LIKE '%unsplash%' OR image_url IS NULL;
...
-- Confirmation : doit afficher 0 lignes
SELECT COUNT(*) as reste_unsplash FROM public.products 
WHERE image_url LIKE '%unsplash%';

SELECT COUNT(*) as reste_unsplash FROM public.recipes 
WHERE image_url LIKE '%unsplash%';
...
-- Mettre à jour les UUIDs des catégories pour correspondre au CSV
UPDATE public.product_categories SET id = 'f8a49f40-1710-5b0e-9df1-e60ce2bc4acc' WHERE slug = 'epicerie-salee';
UPDATE public.product_categories SET id = '34c07ae3-b15d-5a36-8e58-718ee3c4a6bf' WHERE slug = 'produits-laitiers';
UPDATE public.product_categories SET id = '9df4ad90-803c-5100-a325-42b0ed84e1b5' WHERE slug = 'boissons';
UPDATE public.product_categories SET id = '547ad401-05bd-58e4-a4ef-593f975d0499' WHERE slug = 'boulangerie';
UPDATE public.product_categories SET id = 'aed25f4b-771a-52bb-8613-c8134fac3e3e' WHERE slug = 'fruits-legumes';
UPDATE public.product_categories SET id = '78007049-5509-5121-b7df-5428d7299161' WHERE slug = 'epicerie-sucree';
...
-- Remplacer les images Unsplash mortes par Picsum Photos (toujours disponible)
UPDATE products
...
UPDATE recipes  
SET image_url = 'https://picsum.photos/seed/' || id || '/600/400'
WHERE image_url LIKE '%unsplash.com%'
   OR image_url IS NULL
   OR image_url = '';


-- Seulement les tables qui ont image_url
UPDATE products
SET image_url = 'https://picsum.photos/seed/' || id || '/400/300'
WHERE image_url LIKE '%unsplash%';
...
-- Partners
SELECT 'partners - logo_url' as source, id, logo_url as url
FROM partners WHERE logo_url LIKE '%unsplash%';


SELECT 'partners - cover_url' as source, id, cover_url as url
FROM partners WHERE cover_url LIKE '%unsplash%';


-- Articles
SELECT 'articles - image_url' as source, id, image_url as url
FROM articles WHERE image_url LIKE '%unsplash%';


-- Profiles  
SELECT 'profiles - avatar_url' as source, id, avatar_url as url
FROM profiles WHERE avatar_url LIKE '%unsplash%';


-- products image_urls (array)
SELECT id, image_urls 
FROM products 
WHERE image_urls::text LIKE '%unsplash%'
LIMIT 5;
...
-- Assigner diet_tags selon le contenu
UPDATE recipes SET diet_tags = ARRAY[]::text[] WHERE diet_tags IS NULL;

-- faible_ig : lentilles, quinoa, haricots, pois chiches, courgettes
UPDATE recipes SET diet_tags = array_append(diet_tags, 'faible_ig')
WHERE (title ILIKE '%lentil%' OR title ILIKE '%quinoa%' OR title ILIKE '%pois chiche%'
    OR title ILIKE '%haricot%' OR title ILIKE '%courgette%' OR title ILIKE '%avocat%'
    OR description ILIKE '%IG bas%' OR description ILIKE '%diabét%')
AND NOT (diet_tags @> ARRAY['faible_ig']);

-- sans_gluten : sarrasin, quinoa, riz, sans gluten, sans farine
UPDATE recipes SET diet_tags = array_append(diet_tags, 'sans_gluten')
WHERE (title ILIKE '%sarrasin%' OR title ILIKE '%quinoa%' OR title ILIKE '%sans gluten%'
    OR title ILIKE '%sans farine%' OR description ILIKE '%sans gluten%' OR description ILIKE '%cœliaque%')
AND NOT (diet_tags @> ARRAY['sans_gluten']);

-- vegan : salade, légumes, smoothie, soupe végétale
UPDATE recipes SET diet_tags = array_append(diet_tags, 'vegan')
WHERE (title ILIKE '%salade%' OR title ILIKE '%smoothie%' OR title ILIKE '%taboulé%'
    OR title ILIKE '%curry%' OR description ILIKE '%végétalien%' OR description ILIKE '%vegan%')
AND NOT (diet_tags @> ARRAY['vegan']);

-- keto : œufs, fromage, viande, poulet, mousse chocolat
UPDATE recipes SET diet_tags = array_append(diet_tags, 'keto')
WHERE (title ILIKE '%œuf%' OR title ILIKE '%omelette%' OR title ILIKE '%poulet%'
    OR title ILIKE '%mousse%' OR title ILIKE '%fromage%' OR title ILIKE '%wrap%')
AND NOT (diet_tags @> ARRAY['keto']);

-- eco : légumes, salade, soupe, couscous
UPDATE recipes SET diet_tags = array_append(diet_tags, 'eco')
WHERE (title ILIKE '%soupe%' OR title ILIKE '%couscous%' OR title ILIKE '%gratin%'
    OR title ILIKE '%tarte%' OR title ILIKE '%galette%')
AND NOT (diet_tags @> ARRAY['eco']);
...
-- Assigner compatible_with
UPDATE recipes SET compatible_with = ARRAY[]::text[] WHERE compatible_with IS NULL;

-- diabetic
UPDATE recipes SET compatible_with = array_append(compatible_with, 'diabetic')
WHERE (diet_tags @> ARRAY['faible_ig'] OR description ILIKE '%diabét%' OR description ILIKE '%IG bas%')
AND NOT (compatible_with @> ARRAY['diabetic']);

-- celiac
UPDATE recipes SET compatible_with = array_append(compatible_with, 'celiac')
WHERE diet_tags @> ARRAY['sans_gluten']
AND NOT (compatible_with @> ARRAY['celiac']);

-- vegetarian
UPDATE recipes SET compatible_with = array_append(compatible_with, 'vegetarian')
WHERE (diet_tags @> ARRAY['vegan'] OR title ILIKE '%légume%' OR title ILIKE '%quinoa%')
AND NOT (compatible_with @> ARRAY['vegetarian']);


-- Toutes les recettes publiées sont compatibles "healthy"
-- (Mode Sain = pas de restriction spécifique, toutes les recettes conviennent)
UPDATE recipes 
SET compatible_with = array_append(compatible_with, 'healthy')
WHERE NOT (compatible_with @> ARRAY['healthy'])
AND is_published = true;

-- Vérification
SELECT COUNT(*) FROM recipes WHERE compatible_with @> ARRAY['healthy'];
...
-- 1. Voir le profil existant
SELECT id, email, role
FROM public.profiles
WHERE email = 'mez.kad04@gmail.com';

-- 2. Passer le rôle en partner_admin
UPDATE public.profiles
SET role = 'partner_admin'
WHERE email = 'mez.kad04@gmail.com'
RETURNING id, email, role;

-- 3. Vérifier si un partenaire existe déjà pour ce profil
SELECT p.id AS partner_id, p.name, p.owner_id
FROM public.partners p
JOIN public.profiles pr ON pr.id = p.owner_id
WHERE pr.email = 'mez.kad04@gmail.com';

-- 4. Créer le partenaire minimal si aucun n'existe
INSERT INTO public.partners (owner_id, name, country, is_active, is_verified)
SELECT id, 'magazin zaafrania', 'DZ', true, false
FROM public.profiles
WHERE email = 'mez.kad04@gmail.com'
AND NOT EXISTS (
  SELECT 1
...
RETURNING id, name, owner_id, is_active, is_verified;

-- 5. Mettre à jour le nom si le partenaire existe déjà
UPDATE public.partners
SET name = 'magazin zaafrania'
WHERE owner_id = (
  SELECT id
  FROM public.profiles
  WHERE email = 'mez.kad04@gmail.com'
)
AND name <> 'magazin zaafrania'
RETURNING id, name;
...
SELECT p.id AS partner_id, p.name, p.owner_id, pr.email, pr.role
FROM public.partners p
JOIN public.profiles pr ON pr.id = p.owner_id
WHERE pr.email = 'mez.kad04@gmail.com';
...
VALUES (
  '18e37501-cf2e-4f67-8da5-5d48113885c9',
  'kadermezouari02@gmail.com',
  'partner_admin'
)
...
SELECT p.id, p.name, p.owner_id, pr.email, pr.role
FROM public.partners p
JOIN public.profiles pr ON pr.id = p.owner_id
WHERE pr.email = 'kadermezouari02@gmail.com';
...
-- ============================================================
-- SmartHeart - Partner catalog migration
-- ============================================================

-- 1) Table partner_catalog
CREATE TABLE IF NOT EXISTS public.partner_catalog (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  partner_id UUID NOT NULL REFERENCES public.partners(id) ON DELETE CASCADE,
  type TEXT NOT NULL CHECK (type IN ('product', 'recipe')),
  name TEXT NOT NULL,
  data JSONB NOT NULL DEFAULT '{}'::jsonb,
  is_published BOOLEAN NOT NULL DEFAULT false,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT uq_partner_catalog_name UNIQUE (partner_id, type, name)
);

-- 2) Index
CREATE INDEX IF NOT EXISTS idx_partner_catalog_partner_id
  ON public.partner_catalog(partner_id);

CREATE INDEX IF NOT EXISTS idx_partner_catalog_type
  ON public.partner_catalog(type);

CREATE INDEX IF NOT EXISTS idx_partner_catalog_published
  ON public.partner_catalog(is_published)
...
-- 4) Drop trigger if already exists, then recreate
DROP TRIGGER IF EXISTS trg_partner_catalog_updated_at
ON public.partner_catalog;

CREATE TRIGGER trg_partner_catalog_updated_at
BEFORE UPDATE ON public.partner_catalog
FOR EACH ROW
...
-- 6) Policies
DROP POLICY IF EXISTS "Partner reads own catalog" ON public.partner_catalog;
CREATE POLICY "Partner reads own catalog"
  ON public.partner_catalog
  FOR SELECT
  USING (
    partner_id IN (
...
DROP POLICY IF EXISTS "Partner inserts own catalog" ON public.partner_catalog;
CREATE POLICY "Partner inserts own catalog"
  ON public.partner_catalog
  FOR INSERT
  WITH CHECK (
    partner_id IN (
...
DROP POLICY IF EXISTS "Partner updates own catalog" ON public.partner_catalog;
CREATE POLICY "Partner updates own catalog"
  ON public.partner_catalog
  FOR UPDATE
  USING (
    partner_id IN (
...
DROP POLICY IF EXISTS "Partner deletes own catalog" ON public.partner_catalog;
CREATE POLICY "Partner deletes own catalog"
  ON public.partner_catalog
  FOR DELETE
  USING (
    partner_id IN (
...
DROP POLICY IF EXISTS "Public reads published catalog" ON public.partner_catalog;
CREATE POLICY "Public reads published catalog"
  ON public.partner_catalog
  FOR SELECT
...
WHERE table_schema = 'public'
  AND table_name = 'partner_catalog';
...
-- Résultats : accès uniquement à ses propres données
CREATE POLICY "Lecture de ses propres résultats"
  ON public.user_quiz_results FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Insertion de ses propres résultats"
  ON public.user_quiz_results FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Mise à jour de ses propres résultats"
  ON public.user_quiz_results FOR UPDATE
  USING (auth.uid() = user_id);


  -- ── Contrainte UNIQUE pour éviter les doublons de résultats ────
ALTER TABLE public.user_quiz_results
  ADD CONSTRAINT uq_user_quiz UNIQUE (user_id, quiz_id);
...
-- Vérifier si RLS est déjà activé et quelles policies existent
SELECT 
  schemaname, 
  tablename, 
  rowsecurity
FROM pg_tables 
WHERE tablename = 'partner_catalog';
...
CREATE POLICY "Lecture publique du catalogue partenaire"
  ON public.partner_catalog FOR SELECT USING (is_published = true);

CREATE POLICY "Gestion du catalogue par le partenaire"
  ON public.partner_catalog FOR ALL
  USING (EXISTS (
    SELECT 1 FROM public.partners p
    WHERE p.id = partner_id AND p.owner_id = auth.uid()
  ));